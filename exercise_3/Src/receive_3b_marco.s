...
receive_loop:
    ...
    LDR R1, =terminator        	    @Load the user-defined terminating character ('*')
    LDRB R1, [R1]

    CMP R2, R1                  	@Compare received char with terminator
    BEQ receive_done            	@If it matches, the done reveiving

    STRB R2, [R6, R8]         		@Store received character in buffer
    ADD R8, #1              	@Increment buffer index
    CMP R8, R7                 	@Ensure buffer does not overflow
    BGE receive_done            	@if exceeded, it will stop receiving

    B receive_loop              	@Repeat for next character

receive_done:
    STRB R1, [R6, R7]				@Store the terminating character at the end
